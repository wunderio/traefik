		...
		for _, r := range i.Spec.Rules {
			...
			for _, pa := range r.HTTP.Paths {
				...
				for _, port := range service.Spec.Ports {
					if equalPorts(port, intstr.FromInt(int(pa.Backend.Service.Port.Number))) {
...
func (p *Provider) updateIngressStatus(i *netv1.Ingress, k8sClient Client) error {
...
func (p *Provider) addGlobalBackend(cl Client, i *netv1.Ingress, templateObjects *types.Configuration) error {
	...
	for _, port := range service.Spec.Ports {
		...
		if !equalPorts(port, intstr.FromInt(int(i.Spec.DefaultBackend.Service.Port.Number))) {
			continue
		}
...
func getRuleForPath(pa netv1.HTTPIngressPath, i *netv1.Ingress) (string, error) {
	if len(pa.Path) == 0 {
		return "", nil
	}
...
func getTLS(ingress *netv1.Ingress, k8sClient Client, tlsConfigs map[string]*tls.Configuration) error {
...
func getAuthConfig(i *netv1.Ingress, k8sClient Client) (*types.Auth, error) {
...
func getBasicAuthConfig(i *netv1.Ingress, k8sClient Client) (*types.Basic, error) {
...
func getDigestAuthConfig(i *netv1.Ingress, k8sClient Client) (*types.Digest, error) {
	credentials, err := getAuthCredentials(i, k8sClient)
	if err != nil {
		return nil, err
	}
...
func getAuthCredentials(i *netv1.Ingress, k8sClient Client) ([]string, error) {
...
func getForwardAuthConfig(i *netv1.Ingress, k8sClient Client) (*types.Forward, error) {
...
func getFrontendRedirect(i *netv1.Ingress, baseName, path string) *types.Redirect {
...
func getWhiteList(i *netv1.Ingress) *types.WhiteList {
...
func getHeader(i *netv1.Ingress) *types.Headers {
...
func getErrorPages(i *netv1.Ingress) map[string]*types.ErrorPage {
...
func getRateLimit(i *netv1.Ingress) *types.RateLimit {
...
func getPassTLSClientCert(i *netv1.Ingress) *types.TLSClientHeaders {
